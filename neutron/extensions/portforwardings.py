...
class PortForwardingRuleNotFound(qexception.NotFound):
    message = "Port Forwarding Rule %(port_forwarding_rule_id)s " \
              "could not be found."
...
class InvalidProtocol(qexception.InvalidInput):
    message = "Invalid Protocol, allowed value are: tcp, udp"


valid_protocol_values = [None, common_constants.PROTO_NAME_TCP,
                         common_constants.PROTO_NAME_UDP]
...
def _validate_port_range(data, key_specs=None):
    ...
    for p in ports:
        try:
            ...
            msg = "Port '%s' is not a valid number" % p
            LOG.debug(msg)
            return msg
...
def _validate_ip_or_subnet_or_none(data, valid_values=None):
    ...
    if not msg_subnet:
        return None
    return "%(msg_ip)s and %(msg_subnet)s" % \
           {'msg_ip': msg_ip, 'msg_subnet': msg_subnet}
